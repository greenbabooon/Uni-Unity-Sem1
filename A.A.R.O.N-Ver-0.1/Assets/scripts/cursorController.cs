...
public class cursorController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Physics2D.IgnoreCollision(cursorCol, player.GetComponent<Collider2D>());
    ...
    void Update()
    {
        ...
        else if (playerController.GetArmState() == "Carry")
        {
        ...
        if (playerController.GetArmState() == "Carry" || playerController.GetArmState() == "Charged")
        {
            ...
            cursorPos.x = Mathf.Clamp(mousePos.x, player.transform.position.x - maxRad * 1.5f, player.transform.position.x + maxRad * 1.5f);
            cursorPos.y = Mathf.Clamp(mousePos.y, player.transform.position.y - maxRad, player.transform.position.y + maxRad);
        }
        else if (playerController.GetArmState() == "Standard")
        {
        ...
        if (armScript.GetLastCollisionObj2() != null)
        {
            ...
            print("Ignoring collision with " + armScript.GetLastCollisionObj2().name);
        }

        if (Vector2.Distance(cursorPos, player.transform.position) > playerController.GetMaxAimRadius() + 3f)
        {
            cursor.transform.position = player.transform.position;
        }
