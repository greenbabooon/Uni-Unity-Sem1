...
public class playerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        else if (Input.GetKey(KeyCode.Mouse1) && armState != Charged&&GetComponentInChildren<ArmScript>().GetIsCarryDelayed()==false)
        {
            armState = Carry;
        }
        else
        {
            Invoke("DelayedStateChange", 0.1f);
        }
    }
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            Physics2D.IgnoreCollision(collision.gameObject.GetComponent<Collider2D>(), gameObject.GetComponent<CapsuleCollider2D>(), false);
        }
    }
    public void Death()
    {
        FindAnyObjectByType<GameManagerScript>().loadGame();
    }
}
